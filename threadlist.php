...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin']==true){
    ...
    <form action="'.$_SERVER["REQUEST_URI"].' " method="post">
        ...
        <div id="emailHelp" class="form-text">keep your tittle short and crisp as possible.</div>
...
?>
    ...
    <div class="flex-grow-1 ms-3">
        ...
        <h5><a href="thread.php?threadid=' . $th_id . '" class="text-dark">' . $tittle . '</a></h5>
        ' . $thread_desc . '</div> <p class="font-weight-bold my-0" >Asked By :'.$user_name.' at '.$thread_time.'</p>
</div>';
        }
        if ($noResult) {
        ...
        if ($method == 'POST') {
            ...
            $th_desc = $_POST['desc'];
            $sno=$_SESSION['sno'];
            $sql = "INSERT INTO `threads`( `thread_tittle`, `thread_cat_id`, `thread_desc`, `thread_user_id`, `tempdate`)
           VALUES ('$th_tittle','$th_id','$th_desc','$sno',current_timestamp())";
           //$sql="INSERT INTO `comment` (`comment_id`, `comment_content`, `thread_id`, `comment_time`) VALUES ('1', 'this is comment content', '21', CURRENT_TIMESTAMP);";
